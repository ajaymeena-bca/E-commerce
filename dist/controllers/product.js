...
export const getSingleProduct = TryCatch(async (req, res, next) => {
    ...
    if (nodeCache.has(`product-${id}`))
        ...
        if (!product)
            return next(new ErrorHandler("product is not founded", 400));
...
export const newProduct = TryCatch(async (req, res, next) => {
    ...
    if (!photo)
        return next(new ErrorHandler("Please add photo field", 400));
...
export const updateProduct = TryCatch(async (req, res, next) => {
    ...
    if (!product)
        return next(new ErrorHandler("Id is not Invalid", 400));
    if (photo) {
        rm(product.photo, () => {
