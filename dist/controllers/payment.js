...
export const createPaymentIntent = TryCatch(async (req, res, next) => {
    ...
    if (!amount)
        return next(new ErrorHandler("amount must be provided", 404));
...
export const allCoupon = TryCatch(async (req, res, next) => {
    ...
    if (!coupons)
        return next(new ErrorHandler("No Coupons exits", 400));
    res.status(200).json({
        success: true,
...
export const applyDiscount = TryCatch(async (req, res, next) => {
    ...
    if (!coupon)
        return next(new ErrorHandler("coupon not exits", 400));
    res.status(200).json({
        success: true,
